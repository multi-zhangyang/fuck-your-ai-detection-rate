...
function runRegression() {
  ...
  assertIncludes(historySource, "项目体检、空间清理和异常修复都收在这里", "Advanced maintenance copy must keep technical tasks out of the default flow.", failures);
  ...
  assertIncludes(historySource, "先预览再清理", "Current and large asset governance must steer users toward impact preview.", failures);
  ...
  const report = {
    ...
    checks: [
      ...
      "SQL asset governance is surfaced without exposing raw filters",
