...
def main() -> int:
    ...
    if not failed_attempts or failed_attempts[-1].get("outputText") != "错误输出":
        raise AssertionError("targeted rerun fallback must expose failed model output for manual review")
    ...
    if not any(event.get("event") == "validation-retry" and event.get("outputText") == "错误输出" for event in events):
        raise AssertionError("compare payload must record targeted failed outputs")
